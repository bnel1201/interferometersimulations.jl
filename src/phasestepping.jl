...
"""
sample_moire_pattern(proj::PhaseContrastData, rel_step_position=0; ref=false, moire_periods=5, ϕ=0)

rel_step_position: x_n = n/N step position as fraction of 1 period
ref: if true returns a reference moire pattern with no object
ϕ: the system phase  (can be a scalar or 2D array)
"""
function sample_moire_pattern(proj::PhaseContrastData, rel_step_position=0; ref=false, moire_periods=5, ϕ=0)
    M, N = size(proj)
    lateral_position = [2*π*moire_periods*i/M for i = range(1, M, step=1), j = range(1, N, step=1)];
    if ref
        proj = PhaseContrastData(0, 0, 1)
    end
    return 1 .- proj.atten .+ proj.vis.*cos.(2*π*rel_step_position .+ proj.phase .+ lateral_position .+ ϕ)
end
